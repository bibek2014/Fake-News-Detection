...
def Train_Model(Path):
    ...
    corpus1_text = simplify(X_title1, y1)
    ...
    filename = 'Model2_DecisionTreeClassifier_Title.sav'
    joblib.dump(clf, filename)
    # save the CountVectorizer to disk
    filename_cv = 'CountVectorizer_Title.sav'
    joblib.dump(cv, filename_cv)

    # save report in json file
    with open('Model2_DecisionTreeClassifier_Report_Title.json', 'w', encoding='utf-8') as f:
...
def Predict(q):
    ...
    filename = path+'/Model2_DecisionTreeClassifier_Title.sav'
    print(filename)

    clf=joblib.load(filename)
    # Load the CountVectorizer from disk
    filename_cv = path+'/CountVectorizer_Title.sav'
    cv=joblib.load(filename_cv)
    # Load the model report from disk

 
    # Opening JSON file
    f = open(path+'/Model2_DecisionTreeClassifier_Report_Title.json') 
    report = json.load(f)
    ...
    report["Model"]="DecisionTree Classifier"
    return report	
...
if __name__ == "__main__":
    ...
    # Opening JSON file
    f = open('Model2_DecisionTreeClassifier_Report_Title.json') 
    report = json.load(f)
